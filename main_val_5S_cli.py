...
from data_cli import MyDataset
...
results_log_csv_name = 'log_Cli_lr5e4_3FC512_Drop01_Adam_ZJ189.csv'#3FC512
valset_pre_score = 'data/preScoreSx_Cli_lr5e4_3FC512_Drop01_Adam_ZJ189.csv'
# results_log_csv_name = 'log_Cli_lr5e4_3FC512_Drop01_Adam_HZF74.csv'#3FC512
# valset_pre_score = 'data/preScoreSx_Cli_lr5e4_3FC512_Drop01_Adam_HZF74.csv'
...
transform_train = transforms.Compose([
    ...
    transforms.Normalize(mean=[0.2651], std=[0.1188])]) # resize72 jpg [0.2651], std=[0.1188] LS

transform_test = transforms.Compose([
    transforms.Resize((72, 72)),
    transforms.CenterCrop(64),
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.2651], std=[0.1188])])  #resize72 [0.2716], std=[0.1216] LS_HZF

transform_val = transforms.Compose([
    transforms.Resize((72, 72)),
    transforms.CenterCrop(64),
    transforms.ToTensor(), #ZJUF 340 [0.2432], std=[0.1119] #[0.2419], std=[0.1117])]) ZJUF326
    transforms.Normalize(mean=[0.2404], std=[0.1119])]) #val2 ZJUF278 resize72 [0.2404], std=[0.1119] ZJUF

#train and test data
train_data = MyDataset(split = 'train', data_dir=Dataroot, transform=transform_train)
trainloader = torch.utils.data.DataLoader(train_data, batch_size=opt.bs, num_workers=0, shuffle=True, drop_last=True)
test_data = MyDataset(split = 'test', data_dir=Dataroot, transform=transform_test)
testloader = torch.utils.data.DataLoader(test_data, batch_size=16, num_workers=0, shuffle=False)
#external val data
exter_val_data = MyDataset(split = 'val', data_dir=Dataroot_val, transform=transform_val)
exter_valloader = torch.utils.data.DataLoader(exter_val_data, batch_size=16, num_workers=0, shuffle=False)
...
class Densenet_Cli(nn.Module):
    def __init__(self, num_classes=2):
        ...
        FC_size = 512
        self.linear_cli_L1 = nn.Linear(24, FC_size)
